...
void pe_interface_fock(const double* densmat, int ndim, int nnbas, const double* fckmat,
                       double* energy) {
...
void gen1int_api_add_shell(int spher_gto, int idx_center, const double* coord_center,
                           int ang_num, int num_prim, const double* exponents,
                           int num_contr, const double* contr_coef) {
  return gen1int_create_shell_(&spher_gto, &idx_center, coord_center, &ang_num, &num_prim,
...
void pe_set_border_options(int m_pe_border, double m_rmin, int type_flag,
                           int redist_order, int nredist) {
  return pe_set_border_options_(&m_pe_border, &m_rmin, &type_flag, &redist_order,
                                &nredist);
}
