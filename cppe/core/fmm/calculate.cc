...
void interact_dehnen(size_t A, size_t B, std::vector<Cell> &cells, std::vector<Particle> &particles,
  ...
  if (R*theta > (cells[A].rmax + cells[B].rmax)) {
    M2L<M, osize>(dx, dy, dz, cells[B].M, cells[A].L, order);
  }
    ...
    if (cells[B].nleaf >= ncrit) {
      ...
      for(int oa = 0; oa < 8; oa++) {
        if (cells[A].nchild & (1 << oa)) {
          ...
          interact_dehnen<M, osize>(a, B, cells, particles, theta, order, ncrit, F);
        }
      }
  }
...
void interact_dehnen_lazy(const size_t A, const size_t B,
  ...
  if (R*theta > (cells[A].rmax + cells[B].rmax)) {
    ...
    for(int oa = 0; oa < 8; oa++) {
      ...
      if (cells[A].nchild & (1 << oa)) {
    	int a = cells[A].child[oa];
    	interact_dehnen_lazy<m_order, osize>(a, B, cells, particles, theta, order, ncrit, M2L_list, P2P_list);
      }
    }
  }
    ...
    for (unsigned int octant = 0; octant < 8; octant++) {
      ...
      if (cells[p].nchild & (1 << octant)) {
        ...
        if (cells[c].r > theta * r) {
            // If the cell is 'near':
            evaluate_M2P_and_P2P<M, osize>(particles, c, i, cells, F, n_crit, theta, exporder);
        }
        else {
...
template \
void evaluate_P2M<M_ORDER, OUTPUT_SIZE>(std::vector<Particle> &particles, std::vector<Cell> &cells,\
		  size_t cell, size_t ncrit, size_t exporder);\
 \
template \
void evaluate_M2M<M_ORDER, OUTPUT_SIZE>(std::vector<Particle> &particles, std::vector<Cell> &cells, \
                  size_t exporder); \
template \
void evaluate_L2L<M_ORDER, OUTPUT_SIZE>(std::vector<Cell> &cells, size_t exporder);\
 \
template \
void evaluate_L2P<M_ORDER, OUTPUT_SIZE>(std::vector<Particle> &particles, std::vector<Cell> &cells,\
                  double *F, size_t ncrit, size_t exporder);\
 \
template \
void evaluate_direct<M_ORDER, OUTPUT_SIZE>(std::vector<Particle> &particles, double *F, size_t Nparticles);\
 \
template \
void interact_dehnen<M_ORDER, OUTPUT_SIZE>(size_t A, size_t B, std::vector<Cell> &cells, std::vector<Particle> &particles, double theta, size_t order, size_t ncrit, double *F);\
 \
template \
void interact_dehnen_lazy<M_ORDER, OUTPUT_SIZE>(const size_t A, const size_t B, const std::vector<Cell> &cells, const std::vector<Particle> &particles,\
			  const double theta, const size_t order, const size_t ncrit,\
			  std::vector<std::pair<size_t, size_t>> &M2L_list,\
			  std::vector<std::pair<size_t, size_t>> &P2P_list);\
 \
template \
void P2P_Cells<M_ORDER, OUTPUT_SIZE>(size_t A, size_t B, std::vector<Cell> &cells,\
	 			       std::vector<Particle> &particles, double *F);\
 \
template \
void evaluate_M2L_lazy<M_ORDER, OUTPUT_SIZE>(std::vector<Cell> &cells, \
                    std::vector<std::pair<size_t, size_t>> &M2L_list, size_t order); \
 \
template \
void evaluate_P2P_lazy<M_ORDER, OUTPUT_SIZE>(std::vector<Cell> &cells, std::vector<Particle> &particles,\
                    std::vector<std::pair<size_t, size_t>> &P2P_list, double *F);\
 \
template \
void evaluate_M2P_and_P2P<M_ORDER, OUTPUT_SIZE>(std::vector<Particle> &particles, unsigned int p, unsigned int i,\
   std::vector<Cell> &cells, double *F, unsigned int n_crit, double theta, \
   unsigned int exporder);
