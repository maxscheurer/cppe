...
namespace pybind11 {
namespace detail {
template <typename T> struct type_caster<arma::Mat<T>> {
public:
  ...
  bool load(py::handle src, bool convert) {
    ...
    auto buf =
        py::array_t<T, py::array::f_style | py::array::forcecast>::ensure(src);
    if (!buf)
      return false;
...
template <typename T> struct type_caster<arma::Col<T>> {
public:
