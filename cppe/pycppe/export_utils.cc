...
{    
    py::class_<libcppe::PotfileReader> potfile_reader(m, "PotfileReader", "Reader for potential files");
    potfile_reader.def(py::init<std::string>())
                  .def("read", &libcppe::PotfileReader::read, "Read the potential file");
                  
    py::class_<libcppe::PotManipulator> pot_manip(m, "PotManipulator", "Manipulator for potentials");
