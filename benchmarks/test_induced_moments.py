...
def test_induced_moments_solver_scaling(benchmark_recorder, backend, n_polsites):
    ...
    solver = cppe.InducedMoments(polsites, options)
    rhs = np.linspace(0.1, 0.8, 3 * n_polsites)

    repeat, warmup = benchmark_settings()
    timings = time_callable(
        lambda: solver.compute(rhs, True), repeat=repeat, warmup=warmup
    )

    benchmark_recorder(
        benchmark="induced_moments_solver",
        backend=backend,
